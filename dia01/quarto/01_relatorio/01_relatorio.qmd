...
subtitle: "Executando Um Ciclo Simples de Análise de Dados com R + Quarto + Rstudio"
author: SEU NOME
lang: pt
format:
  html:
...
```{r}
# Filtrar todas as vendas realizadas na cidade de "Formiga"
...
```{r}
# Selecionar as colunas Data, Cidade e Receita
...
```{r}
# Agrupar as vendas por cidade e calcular a receita total por cidade
...
```{r}
# Filtrar vendas realizadas em "Formiga" e com Receita maior que 1000
...
```{r}
# Contar o número de transações realizadas por cada representante
...
```{r}
# Calcular a receita total por representante e ordenar em ordem decrescente
...
```{r}
# Calcular a média de unidades vendidas por produto e cidade
...
```{r}
# Calcular a receita média por representante e por produto
...
```{r}
# Criar tabela usando o pacote gt 
tabela_receita %>%
  gt() %>%
  # Adiciona um cabeçalho à tabela com título e subtítulo
  tab_header(
...
```{r}
# Criar a tabela interativa com reactable
reactable(
  tabela_receita,
  pagination = FALSE,
...
# Cria o gráfico de barras horizontal ordenado
